        ...
        <div class="row" style="margin-top: 30px;">
            <div class="column_3">
                ...
                <div id="chongia">
                    ...
                    <div id="nutapdung" class="chonkhoanggia" onclick="chongia('<?php echo $_GET['loai'] ?>','<?php echo $_GET['text'] ?>')">Áp dụng</div>
                </div>
            </div>
            <div class="column_9">
                ...
                <div class="row" id="product-item">
                    ...
                    include("modules/admin/config.php");
                    $loai = $_GET['loai'];
                    $sql = "SELECT * from tbl_sanpham WHERE tensp LIKE N'%$search%' AND matheloai = $loai and trangthai =1";
            ...
            <div id="trang">
                ...
                if ($dem > 6) {
                    for ($i = 1; $i <= ceil($dem / 6); $i++) {
                        if ($i == 1) {
                            echo "<a id='btn_phantrang" . $i . "'  class='slide active1' onclick='chuyentrang(`" . $i . "`,`" . $search . "`,`" . $loai . "`)'>" . $i . "</a>&ensp;";
                        } else {
                            echo "<a id='btn_phantrang" . $i . "'  class='slide' onclick='chuyentrang(`" . $i . "`,`" . $search . "`,`" . $loai . "`)'>" . $i . "</a> &ensp;";
                        }
                    }
                    echo "</div></div>";
                }
...
<script>
    function chuyentrang(sotrang, search, loai) {
        $.post("modules/ajax.php", {
            sotrang: sotrang,
            search: search,
            loai: loai,
            action: "phantrangsearchloai"
        }, function(result) {
            var dem = 1;
            $("#row_product_1").html('');
            $("#row_product_2").html('');
    ...
    function chongia(loai, search) {
        ...
        location.href = "index.php?action=giasp&loaisp=" + loai + "&text=" + search + "&giathap=" + giathap + "&giacao=" + giacao;
    }
</script>