        ...
        <div class="row" style="margin-top: 30px;">
            <div class="column_3">
                            ...
                            if(isset($_GET['action'])){
                                if($_GET['action']=='search'){
                                    ...
                                    while ($row = $query->fetch_array()) {
                                        echo "<a href='index.php?action=search&text=".$_GET['text']."&loaisp=" . $row['matheloai'] . "'><li>" . $row['tentheloai'] . "</li></a>";
                                    }
                                } else{
                                    include("modules/admin/config.php");
                                    $sql =  "SELECT * FROM tbl_theloai";
                ...
                <div id="chongia">
                    ...
                    <div id="nutapdung" class="chonkhoanggia" onclick="chongia('<?php echo $_GET['loaisp'] ?>')">Áp dụng</div>
                </div>
            </div>
            <div class="column_9">
                <div class="row" id="product-item">
                    ...
                    include("modules/admin/config.php");
                    $loai = $_GET['loaisp'];
                    $sql = "SELECT * from tbl_sanpham WHERE matheloai = $loai and trangthai = 1";
            ...
            <div id="trang">
                ...
                if ($dem > 6) {
                    for ($i = 1; $i <= ceil($dem / 6); $i++) {
                        if ($i == 1) {
                            echo "<a id='btn_phantrang" . $i . "'  class='slide active1' onclick='chuyentrang(`" . $i . "`,`" . $loai . "`)'>" . $i . "</a>&ensp;";
                        } else {
                            echo "<a id='btn_phantrang" . $i . "'  class='slide' onclick='chuyentrang(`" . $i . "`,`" . $loai . "`)'>" . $i . "</a> &ensp;";
                        }
                    }
                    echo "</div></div>";
                }
...
<script>
    function chuyentrang(sotrang, loai) {
        $.post("modules/ajax.php", {
            sotrang: sotrang,
            loai: loai,
            action: "phantrangloai"
        }, function(result) {
            var dem = 1;
            $("#row_product_1").html('');
            $("#row_product_2").html('');
    ...
    function chongia(idloai) {
        ...
        location.href = "index.php?action=giasp&loaisp=" + idloai + "&giathap=" + giathap + "&giacao=" + giacao;
    }
</script>