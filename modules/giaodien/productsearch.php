        ...
        <div class="row" style="margin-top: 30px;">
            <div class="column_3">
                    ...
                    <ul id="categary">
                        ...
                        if (isset($_GET['action'])) {
                            if ($_GET['action'] == 'search') {
                                ...
                                $query = mysqli_query($connect, $sql);
                                while ($row = $query->fetch_array()) {
                                    echo "<a href='index.php?action=search&text=" . $_GET['text'] . "&loai=" . $row['matheloai'] . "'><li>" . $row['tentheloai'] . "</li></a>";
                                }
                            } else {
                                include("modules/admin/config.php");
                                $sql =  "SELECT * FROM tbl_theloai";
                ...
                <div id="chongia">
                    ...
                    <div id="nutapdung" class="chonkhoanggia" onclick="chongia('<?php echo $_GET['text'] ?>')">Áp dụng</div>
                </div>
            </div>
            <div class="column_9">
                ...
                <div class="row" id="product-item">
                    ...
                    $dem = 1;
                    include("modules/admin/config.php");
                    $sql = "SELECT * from tbl_sanpham WHERE tensp LIKE N'%$search%' and trangthai = 1";
            ...
            <div id="trang">
                ...
                if ($dem > 6) {
                    for ($i = 1; $i <= ceil($dem / 6); $i++) {
                        if ($i == 1) {
                            echo "<a id='btn_phantrang" . $i . "'  class='slide active1' onclick='chuyentrang(`" . $i . "`,`" . $search . "`)'>" . $i . "</a>&ensp;";
                        } else {
                            echo "<a id='btn_phantrang" . $i . "'  class='slide' onclick='chuyentrang(`" . $i . "`,`" . $search . "`)'>" . $i . "</a> &ensp;";
                        }
                    }
                    echo "</div></div>";
                }
...
<script>
    function chuyentrang(sotrang, search) {
        $.post("modules/ajax.php", {
            sotrang: sotrang,
            search: search,
            action: "phantrangsearch"
        }, function(result) {
            var dem = 1;
            $("#row_product_1").html('');
            $("#row_product_2").html('');
    ...
    function chongia(search) {
        ...
        location.href = "index.php?action=giasp&text=" + search + "&giathap=" + giathap + "&giacao=" + giacao;
    }
</script>